...
async def send_text_to_room(
    ...
    for message in messages:
        ...
        if code:
            content["formatted_body"] = "<pre><code>" + \
                message + "</code></pre>"
...
async def send_image_to_rooms(client, rooms, image):
    ...
    if not os.path.isfile(image):
        logger.debug(f"File {image} is not a file. Doesn't exist or "
                     "is a directory."
                     "This file is being droppend and NOT sent.")
        return
    ...
    if not mime_type.startswith("image/"):
        logger.debug("Drop message because file does not "
    ...
    try:
        for room_id in rooms:
            ...
            logger.debug(f"This image was sent: \"{image}\" "
                         f"to room \"{room_id}\".")
    except Exception:
...
async def send_file_to_rooms(client, rooms, file):
    ...
    if (isinstance(resp, UploadResponse)):
        ...
        logger.info("Bot failed to upload. "
